...
class checkDemoAuth{
    ...
    func checkCertification() -> intptr_t {
        ...
        // 認証済みフラグ存在確認
        let sql1 = "SELECT count(*) FROM demo_certification;"
        ...
        if data_count == 0{
            ...
            let sql = "INSERT INTO demo_certification (certification_flag, created, modified) VALUES (?, ?, ?);"
            ...
            // 認証フラグ取得
            let sql_menu = "SELECT certification_flag FROM demo_certification;"
    ...
    func setCertification() {
        ...
        db_usedb.open()
        db_usedb.beginTransaction()
        let sql:String = "UPDATE demo_certification SET certification_flag = 1;"
