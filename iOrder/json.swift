...
extension JSON {
    ...
    public var allValues:JSON{
        if((self._value as AnyObject).allValues == nil) {
            return JSON([])
        }
        return JSON((self._value as AnyObject).allValues)
    }
    // gives all keys content in JSON object.
    public var allKeys:JSON{
        if((self._value as AnyObject).allKeys == nil) {
            return JSON([])
        }
        return JSON((self._value as AnyObject).allKeys)
    }
}
extension JSON : Sequence {
    ...
    public func mutableCopyOfTheObject() -> Any {
        return (_value as AnyObject).mutableCopy()
    }
}
